...
{
    [McpServerTool(Name = "get_goals"), Description("Get all active goals for the current user")]
    public Task<List<Goal>> GetGoals(string userJwt, CancellationToken ct)
    {
        return dataService.GetGoalsAsync(userJwt, ct);
    }

    [McpServerTool(Name = "get_tasks"), Description("Get active tasks, optionally filtered by goal ID")]
    ...
    [McpServerTool(Name = "create_goal"), Description("Create a new goal")]
    ...
    [McpServerTool(Name = "create_task"), Description("Create a new task")]
    ...
    [McpServerTool(Name = "complete_task"), Description("Mark a task as complete (archives it)")]
    ...
    [McpServerTool(Name = "propose_goals"), Description("Propose structured goals with linked tasks for user review")]
    public string ProposeGoals(List<ProposedGoal> goals)
    {
        return JsonSerializer.Serialize(new GoalProposal(goals));
    }

    [McpServerTool(Name = "brain_dump"), Description("Decompose a brain dump into clarifying questions or goal/task proposals")]
